...
export default function BlogPage() {
  ...
  const [form, setForm] = useState({ title: '', site_id: '', lang: 'de', status: 'draft', content: '', excerpt: '' })
  ...
  async function addPost(e: React.FormEvent) {
    ...
    if (r.ok) { setPosts(p => [d, ...p]); setShowAdd(false); setForm({ title: '', site_id: '', lang: 'de', status: 'draft', content: '', excerpt: '' }) }
    setSaving(false)
  }
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18, padding: 32, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={addPost}>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 6 }}>Zusammenfassung</label>
                <textarea style={{ ...inputStyle, height: 64, resize: 'none' } as React.CSSProperties} value={form.excerpt} onChange={e => setForm(f => ({ ...f, excerpt: e.target.value }))} placeholder="Kurze Zusammenfassung…" />
              </div>
              <div style={{ marginBottom: 24 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 6 }}>Inhalt</label>
                <textarea style={{ ...inputStyle, height: 140, resize: 'vertical' } as React.CSSProperties} value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} placeholder="Blog-Inhalt (HTML oder Markdown)…" />
              </div>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[1,2,3].map(i => <div key={i} style={{ height: 60, borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border)' }} className="skeleton" />)}
        </div>
      ) : (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 0.7fr 0.7fr 120px', gap: 0, padding: '10px 18px', borderBottom: '1px solid var(--border)' }}>
            {['Titel', 'Website', 'Sprache', 'Status', 'Aktionen'].map(h => (
          ...
          {filtered.length === 0 ? (
            <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text3)', fontSize: 14 }}>Keine Posts gefunden</div>
          ) : filtered.map((post, i) => (
            <div key={post.id} style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 0.7fr 0.7fr 120px', gap: 0, padding: '12px 18px', borderBottom: i < filtered.length - 1 ? '1px solid rgba(31,36,56,0.5)' : 'none', alignItems: 'center' }}>
              ...
              <div style={{ fontSize: 12, fontFamily: 'Space Mono, monospace', color: 'var(--text3)' }}>{post.lang?.toUpperCase()}</div>
              <div>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => toggleStatus(post)} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text3)', cursor: 'pointer', fontSize: 11, fontFamily: 'inherit' }}>
                  {post.status === 'published' ? '↓ Entwurf' : '↑ Live'}
                </button>
                <button onClick={() => deletePost(post.id)} style={{ padding: '5px 8px', borderRadius: 6, border: '1px solid rgba(239,68,68,0.2)', background: 'rgba(239,68,68,0.05)', color: '#ef4444', cursor: 'pointer', fontSize: 12 }}>✕</button>
              </div>
            </div>
          ))}
        </div>
